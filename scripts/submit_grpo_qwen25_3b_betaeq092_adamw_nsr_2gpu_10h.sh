...
# Uses AdamW_NSR optimizer (NSR state representation).
# - log_adam_snr=True (reads r=NSR² directly from optimizer state)
# - ppo_epochs=3 (default from run_qwen2.5-3b_math_grpo.sh)
# - No LR override (uses run script default)
#
# Usage (on host):
#   bash verl_v0.4.x/scripts/submit_grpo_qwen25_3b_betaeq092_adamw_nsr_2gpu_10h.sh
...
echo "============================================================"
echo "[qwen-adamw-nsr] partition=${PARTITION} gpus=${GPUS} time=${TIME} mem=${MEM} cpus=${CPUS}"
echo "[qwen-adamw-nsr] seed=${SEED} project=${PROJECT_NAME}"
echo "[qwen-adamw-nsr] model=${MODEL_PATH}"
echo "[qwen-adamw-nsr] optimizer=AdamW_NSR betas=[${beta1},${beta2}] log_adam_snr=True"
echo "[qwen-adamw-nsr] mixed_precision_tag=${MP_TAG} model_dtype=${FSDP_MODEL_DTYPE} train_dtype=${FSDP_TRAIN_DTYPE} param/reduce/buffer=${FSDP_PARAM_DTYPE}/${FSDP_REDUCE_DTYPE}/${FSDP_BUFFER_DTYPE} use_orig_params=True"
echo "[qwen-adamw-nsr] img=${IMG}"
echo "[qwen-adamw-nsr] bind=${BIND}"
echo "[qwen-adamw-nsr] run_script=${RUN_SCRIPT}"
echo "============================================================"
...
echo "[cfg] optimizer=AdamW_NSR betas=[${beta1},${beta2}] seed=${SEED} exp_name=${exp_name}"
...
echo "============================================================"
echo "[submitted] job_id=${jid} name=${job_name} optimizer=AdamW_NSR betas=[${beta1},${beta2}]"
echo "Tips:"
