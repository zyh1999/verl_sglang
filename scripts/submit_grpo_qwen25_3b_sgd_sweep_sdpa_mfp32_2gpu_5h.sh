...
# Sweep SGD vs Momentum-SGD for ppo_epochs=1 under:
...
#
# Notes:
# - Uses torch.optim.SGD via verl dynamic optimizer import.
# - Momentum variant is implemented by setting override_optimizer_config.momentum=0.9.
...
ONLY_MOMENTUM="${ONLY_MOMENTUM:-0}"
opts=(SGD SGD_Momentum)
if [[ "${ONLY_MOMENTUM}" == "1" ]]; then
  opts=(SGD_Momentum)
fi
...
for opt in "${opts[@]}"; do
  for lr in "${lrs[@]}"; do
    ...
    if [[ "${opt}" == "SGD" ]]; then
      ...
      opt_tag="SGD_mom${MOMENTUM}"
      opt_tag="${opt_tag//./p}"
    fi

    mp_tag="sdpa_amp_bf16_mfp32_${opt_tag}_lr${lr_tag}_pe${PPO_EPOCHS}"
    ...
    if [[ "${opt}" == "SGD_Momentum" ]]; then
      # override_optimizer_config is None by default (struct mode), so we must append the key with '+'.
      hydra_args+=("+actor_rollout_ref.actor.optim.override_optimizer_config.momentum=${MOMENTUM}")
    fi

    hydra_args_escaped="$(printf " %q" "${hydra_args[@]}")"

    echo "------------------------------------------------------------"
    echo "[submit] opt=${opt_tag} lr=${lr} ppo_epochs=${PPO_EPOCHS} attn=${ATTN_IMPL} m=${FSDP_MODEL_DTYPE} d=${FSDP_TRAIN_DTYPE}"
...
echo "[cfg] opt=${opt_tag} lr=${lr} exp_name=${exp_name}"
...
echo "Tip: squeue -u \$USER | rg \"SGD_mom|sdpa_amp_bf16_mfp32_SGD\""
echo "------------------------------------------------------------"
