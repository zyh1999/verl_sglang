...
# Sweep Momentum-SGD for ppo_epochs=1 under:
# - attn_implementation=sdpa
# - params fp32 (model_dtype=fp32)
# - train compute bf16 (fsdp_config.dtype=bfloat16)
# - 2 GPU, 5h
#
# Submits 20 jobs total:
#   4 momentum values x 5 larger learning rates
...
# "更大的 lr"：从 1e-5 往上扫 5 个档位
lrs=(2e-5 3e-5 5e-5 1e-4 2e-4)

# 不同 momentum 系数：4 个值，共 20 组
...
for mom in "${momentums[@]}"; do
  for lr in "${lrs[@]}"; do
    ...
    hydra_args=(
      ...
      "+actor_rollout_ref.actor.optim.override_optimizer_config.momentum=${mom}"
    ...
    echo "------------------------------------------------------------"
    echo "[submit] mom=${mom} lr=${lr} ppo_epochs=${PPO_EPOCHS} attn=${ATTN_IMPL} m=${FSDP_MODEL_DTYPE} d=${FSDP_TRAIN_DTYPE}"
...
echo "[cfg] opt=${opt_tag} lr=${lr} momentum=${mom} exp_name=${exp_name}"
...
echo "Tip: squeue -u \$USER | rg \"SGD_mom0p|sdpa_amp_bf16_mfp32_SGD_mom\""
echo "------------------------------------------------------------"
