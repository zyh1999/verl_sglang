...
# Submit one GRPO job: AdamW_NSR + beta1=beta2 + ppo_epochs=1 + mixed precision.
...
BETA2="${BETA2:-0.92}"
ATTN_IMPL="${ATTN_IMPL:-}"
MP_TAG="${MP_TAG:-amp_bf16_m${FSDP_MODEL_DTYPE}_adamw_nsr_b1eqb2_0p92_pe${PPO_EPOCHS}_adam_snr}"
MP_TAG="${MP_TAG//./p}"
...
echo "============================================================"
echo "[qwen-adamw-nsr-mixed] partition=${PARTITION} gpus=${GPUS} time=${TIME} mem=${MEM} cpus=${CPUS}"
echo "[qwen-adamw-nsr-mixed] seed=${SEED} project=${PROJECT_NAME}"
echo "[qwen-adamw-nsr-mixed] model=${MODEL_PATH}"
echo "[qwen-adamw-nsr-mixed] optimizer=AdamW_NSR betas=[${BETA1},${BETA2}] log_adam_snr=True"
echo "[qwen-adamw-nsr-mixed] ppo_epochs=${PPO_EPOCHS}"
echo "[qwen-adamw-nsr-mixed] mixed_precision_tag=${MP_TAG} model_dtype=${FSDP_MODEL_DTYPE} train_dtype=${FSDP_TRAIN_DTYPE}"
echo "[qwen-adamw-nsr-mixed] img=${IMG}"
echo "[qwen-adamw-nsr-mixed] run_script=${RUN_SCRIPT}"
echo "============================================================"
...
echo "[cfg] adamw_nsr_mixed seed=${SEED} exp_name=${exp_name}"
