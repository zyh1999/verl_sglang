...
# Submit one GRPO job with end-to-end fp32 intent (train + rollout).
...
#
# Usage:
#   bash verl_v0.4.x/scripts/submit_grpo_qwen25_3b_betaeq092_fp32_e2e_sglang_2gpu_10h.sh
...
echo "============================================================"
echo "[qwen-fp32-e2e] partition=${PARTITION} gpus=${GPUS} time=${TIME} mem=${MEM} cpus=${CPUS}"
echo "[qwen-fp32-e2e] seed=${SEED} project=${PROJECT_NAME}"
echo "[qwen-fp32-e2e] model=${MODEL_PATH}"
echo "[qwen-fp32-e2e] betas=[${beta1},${beta2}] log_adam_snr=True"
echo "[qwen-fp32-e2e] mp_tag=${MP_TAG}"
echo "[qwen-fp32-e2e] actor/ref model_dtype=${FSDP_MODEL_DTYPE} train_dtype=${FSDP_TRAIN_DTYPE}"
echo "[qwen-fp32-e2e] rollout=${ROLLOUT_NAME} rollout.dtype=${ROLLOUT_DTYPE} attn=${ATTN_IMPL}"
echo "[qwen-fp32-e2e] img=${IMG}"
echo "[qwen-fp32-e2e] run_script=${RUN_SCRIPT}"
echo "============================================================"
...
echo "[cfg] fp32-e2e seed=${SEED} exp_name=${exp_name}"
